...
// CheckOrCreate looks up a device by its broadcast id (the IMEI a GT06 device
// emits on login). A new device is auto-created as `pending` and allowed to
// connect; a `blocked` device is rejected; everything else (pending/active) is
// allowed. If s is nil (DB disabled), every device is treated as approved.
func (s *DeviceStore) CheckOrCreate(ctx context.Context, broadcastID, model string) (CheckResult, error) {
	...
	name := broadcastID
	notes := fmt.Sprintf("Auto-registered via GT06. Model: %s", model)
